...
def subject_edit(request, pk):
    ...
    # Only the teacher who owns this subject can edit
    if not profile.is_teacher() or subject.teacher != request.user:
        messages.error(request, "You are not allowed to edit this subject.")
    ...
    # All verified students ordered by branch then name, for the filter UI
    all_students = (
        Profile.objects
        .filter(role='student', is_verified=True)
        .select_related('user')
        .order_by('branch', 'user__first_name', 'user__last_name')
    )
